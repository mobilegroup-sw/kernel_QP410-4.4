...
#define LCD_DEBUG 1
#if LCD_DEBUG
#define lcd_pr_err	pr_err
#else
#define lcd_pr_err pr_debug
#endif
...
//	pr_debug
	...
	if (enable) {
		...
		lcd_pr_err("%s: pinfo->panel_power_on = %d\n", __func__, pinfo->panel_power_on);
		if (!pinfo->panel_power_on) {			// lee modify, 20150103                   
			if (gpio_is_valid(ctrl_pdata->disp_en_gpio))
