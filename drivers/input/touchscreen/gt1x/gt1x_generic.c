...
struct mutex gesture_doze_lock; /*kyle: add lock for gesture_doze_status */
...
{
	char * temp_buf = (char*)kzalloc( 4096,GFP_KERNEL);
	char *ptr = temp_buf; 
	...
	count = simple_read_from_buffer(page, size, ppos, temp_buf, 4096);
	kfree(temp_buf);
	return count;
}
...
#if !GTP_ESD_PROTECT && !GTP_POWER_CTRL_SLEEP 
...
s32 gt1x_init_panel(void)
{
	...
	if (cfg_len < GTP_CONFIG_MIN_LENGTH || cfg_len > GTP_CONFIG_MAX_LENGTH) {
		GTP_ERROR("Config group%d is INVALID! You need to check you header file CFG_GROUP section!", sensor_id);
		return -1;
	}
	...
	gt1x_config[0] &= 0x7F;	//0xFF will clear the all parameters which was fixed in the flash.
...
#define _cfg_array_(n)   GTP_CHARGER_CFG_GROUP##n
  ...
  GTP_INFO("gt1x_parse_chr_cfg(): start sensor_id=%d", sensor_id);
    ...
    do{
       ...
       len = cfg_lens[sensor_id];
		...
		GTP_INFO("gt1x_parse_chr_cfg(): sensor_id=%d", sensor_id);
...
#if GTP_GESTURE_WAKEUP
	mutex_init(&gesture_doze_lock);	//kyle:add
#endif
