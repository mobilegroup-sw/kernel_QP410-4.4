...
#define FT_REG_FW_MAJ_VER	0xB1	// add 1225
...
static void ft5x06_update_fw_ver(struct ft5x06_ts_data *data)
{
	...
	reg_addr = FT_REG_FW_MAJ_VER;		// modify 1225
...
//		pr_err("%s: x:%x,y:%x\n", __func__,x,y);		//add. debug.
//		pr_err("%s: x_end:%x,y_end:%x\n", __func__,x_end,y_end);
...
//			input_report_abs(ip_dev, ABS_MT_POSITION_Y, y);
			input_report_abs(ip_dev, ABS_MT_POSITION_X, x_end);
			input_report_abs(ip_dev, ABS_MT_POSITION_Y, y_end);
		} else {
			input_mt_report_slot_state(ip_dev, MT_TOOL_FINGER, 0);
		}
	}
//	pr_err("%s: update_input:%x\n", __func__,update_input);			//add. debug.
	if (update_input) {
			   ...
			   const struct i2c_device_id *id)
{
	...
	err = device_create_file(&client->dev, &dev_attr_ts_info);		// add 1225
	if (err) {
...
static int ft5x06_ts_remove(struct i2c_client *client)
{
	...
	device_remove_file(&client->dev, &dev_attr_update_fw);
	device_remove_file(&client->dev, &dev_attr_fw_name);
	device_remove_file(&client->dev, &dev_attr_enable);					// add 1225
	device_remove_file(&client->dev, &dev_attr_mt_protocol_type);		// add 1225
	device_remove_file(&client->dev, &dev_attr_ts_info);				// add 1225
