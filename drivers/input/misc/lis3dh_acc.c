...
/* Debug Message Flags */
#define LIS3DH_KMSG_ERR	0	/* Print kernel debug message for error */
#define LIS3DH_KMSG_INF	0	/* Print kernel debug message for info */

#if LIS3DH_KMSG_ERR
#define LIS3DHMSGERR(format, ...)	\
...
static int lis3dh_acc_config_regulator(struct lis3dh_acc_data *acc, bool on)
{
	...
	LIS3DHMSGERR(&acc->client->dev, "%s: on %d\n", __func__, on);
	if (on) {
		for (i = 0; i < num_reg; i++) {
		...
		int *xyz)
{
	...
	#ifdef DEBUG
	LIS3DHMSGERR(&acc->client->dev, "%s read x=%d, y=%d, z=%d\n",
				     ...
				     const char *buf, size_t size)
{
	...
	mutex_unlock(&acc->lock);
	LIS3DHMSGERR(&acc->client->dev, "%s: poll_interval %d\n", __func__, acc->pdata->poll_interval);
	return size;
}
			      ...
			      const char *buf, size_t size)
{
	...
	mutex_unlock(&acc->lock);
	LIS3DHMSGERR(&acc->client->dev, "%s: g_range %d\n", __func__, acc->pdata->g_range);
	return size;
}
			       ...
			       struct device_attribute *attr, char *buf)
{
	...
	LIS3DHMSGERR(&acc->client->dev, "%s: enabled %d\n", __func__, val);
			       ...
			       const char *buf, size_t size)
{
	...
	LIS3DHMSGERR(&acc->client->dev, "%s: enabled %ld\n", __func__, val);
	if (val)
	...
	unsigned int enable)
{
	...
	LIS3DHMSGERR(&acc->client->dev, "%s: enable %d\n", __func__, enable);
	if (enable)
