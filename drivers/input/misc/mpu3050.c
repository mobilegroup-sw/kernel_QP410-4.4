...
/* Debug Message Flags */
#define MPU_KMSG_ERR	0	/* Print kernel debug message for error */
#define MPU_KMSG_INF	0	/* Print kernel debug message for info */

#if MPU_KMSG_ERR
#define KMSGERR_MPU(format, ...)	\
				...
				const char *buf, size_t size)
{
	...
	KMSGERR_MPU(dev, "%s:interval_ms %ld\n", __func__, interval_ms);
		...
		unsigned int enabled)
{
	...
	KMSGERR_MPU(&sensor->client->dev, "%s:%d %d\n", __func__, enabled, sensor->enable);
...
error:
	...
	KMSGERR_MPU(dev, "%s:Unable to create interface\n", __func__);
	return err;
}
			     ...
			     struct axis_data *coords)
{
	...
	coords->x = coords->x + 78;
	coords->y = coords->y + 29;
	coords->z = coords->z - 24;
#endif
				   ...
				   const struct i2c_device_id *id)
{
	...
	dev_err(&client->dev, "mpu3050_probe\n");
...
*/
	...
	if (ret < 0) {
		dev_err(&client->dev, "failed to detect device ret %d\n", ret);
		error = -ENXIO;
	...
	if (client->irq == 0) {
		...
		if (gpio_is_valid(sensor->platform_data->gpio_int)) {
			...
			pr_err("%s: enable to request for gpio %d\n",
			...
			client->irq = gpio_to_irq(
					sensor->platform_data->gpio_int);
			pr_err("%s: enable to request irq %d\n",
				__func__, client->irq);
		} else {
			ret = -EINVAL;
			goto err_pm_set_suspended;
