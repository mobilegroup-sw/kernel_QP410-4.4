...
#include <linux/spi/jmt301.h>
#include <linux/spi/jmt.h>
#include <linux/spi/jmt_ioctl.h>

#include "mt_spi.h"

#define DRIVER_VERSION                 "1.0.9.0"
...
#define JMT101_RESET_GPIO              (GPIO116 | 0x80000000)
#else
#define EINT0                          3
#define JMT101_INT_IRQNO               EINT0
#define JMT101_IRQ_GPIO                (19 | 0x80000000)
#define JMT101_RESET_GPIO              (116 | 0x80000000)
#endif
...
static struct mt_chip_conf spi_conf_mt6577 = {
   .setuptime = 3,
   .holdtime = 3,
   .high_time = 16,  // for mt6589, 100000khz/(4+4) = 125000khz
   .low_time = 16,
   .cs_idletime = 2,
   .ulthgh_thrsh = 0,
...
/*
static struct spi_board_info jmt101_board_info __initdata = {
    .modalias = "jmt101",
    .platform_data = NULL,
    .mode = SPI_MODE_3,
...
static struct of_device_id jmt101_match_table[] = {
...
static struct spi_driver jmt101_driver = {
   .driver = {
      ...
      .bus = &spi_bus_type,
      .owner = THIS_MODULE,
      .of_match_table = jmt101_match_table,
      .pm = &jmt101_pm,
   },
   .probe = jmt101_probe,
...
//	if (data->power_on != on) {
		if (!IS_ERR_OR_NULL(data->pinctrl)) {
			if (on){
				gpio_set_value(data->finger_ext_3v3_en_gpio, 1);
//				err = pinctrl_select_state(data->pinctrl,
//					data->pin_default);
			} else {
				gpio_set_value(data->finger_ext_3v3_en_gpio, 0);
//				err = pinctrl_select_state(data->pinctrl,
//					data->pin_sleep);
				}
//			if (err)
...
static int jmt101_pinctrl_init(struct jmt101_data *data)
{
	struct spi_device *client = data->spi;
...
/* -------------------------------------------------------------------- */
/*static void jmt101_power_set(int switch_val, struct jmt101_data *jmt101)
{
	printk ("jmt101_power_set switch_val %d\n", switch_val);
	if (switch_val) {
		gpio_set_value(jmt101->shoutdown_gpio, 1);
		udelay (1000);
		gpio_set_value(jmt101->finger_ext_3v3_en_gpio, 1);
		udelay (1000);
		gpio_set_value(jmt101->reset_gpio, 0);
		udelay (1000);
		gpio_set_value(jmt101->reset_gpio, 1);
		udelay (1250);
	} else {
		gpio_set_value(jmt101->finger_ext_3v3_en_gpio, 0);
		gpio_set_value(jmt101->reset_gpio, 0);
	}
}*/
...
static int jmt101_spi_rd_reg (struct jmt101_data *jmt101, u8 addr, u8 *value)
{
    ...
    *value = spi_buf[1];
//    dev_err (&jmt101->spi->dev, "jmt101_spi_rd_reg addr 0x%x, return value 0x%x.\n",addr, *value);
    return (error);
}
...
//	dev_err (&jmt101->spi->dev, "jmt101_spi_wr_reg addr 0x%x, return value 0x%x.\n",addr, value);
   return (error);
}
...
static int jmt101_reset (struct jmt101_data *jmt101)
{
   int error;
/*
#ifdef CONFIG_SPI_FINGERPRINT_JMS301A
	gpio_set_value(jmt101->reset_gpio, 0);
	udelay (1000);
	gpio_set_value(jmt101->reset_gpio, 1);
	udelay (1250);
...
//    dev_err (&jmt101->spi->dev, "arrange_frame enter. jmt101->sa.version %d\n", jmt101->sa.version);
	/* handle error ... */
...
//	dev_err (&jmt101->spi->dev, "calc_intensity enter.\n");
...
//	dev_err (&jmt101->spi->dev, "add_vrb enter.\n");
   do {
      if (add_no <= 0) {
         break;
      }
...
int sub_vrb (struct jmt101_data* jmt101, int sub_no)
{
   ...
   int bias = (!jmt101->sa.version)?12:3;
//   dev_err (&jmt101->spi->dev, "sub_vrb enter.\n");
   do {
      if (sub_no <= 0) {
         break;
      }
...
//   dev_err (&jmt101->spi->dev, "do_calibrate enter.\n");
    switch(sa->version){
        case 0:
...
{
    u8 data = 0;
//	dev_err (&jmt101->spi->dev, "check_dvr_enable enter.\n");
...
//   dev_err (&jmt101->spi->dev, "jmt101_spi_rd_image enter.\n");
   if(sa->version == 0) {
       tr_dat1.cs_change = 1;
       ...
       tr_dat1.speed_hz = jmt101->spi_speed;
       tr_dat1.tx_buf = spi_cmd;
       tr_dat1.rx_buf = jmt101->huge_buffer + rx_off +1;
       tr_dat1.len = sa->sensor_size+1;
    /*
       tr_dat2.cs_change = 1;
...
//   dev_err (&jmt101->spi->dev, "get huge buffer sa->version %d.\n", sa->version);
   /* re-organize frame data */
	if(sa->version < 3) 	arrange_frame (jmt101, jmt101->huge_buffer + rx_off + 1);
#if 1
    switch(sa->version){
        case 0:
            buf_source = 1;
...
//	dev_err (&jmt101->spi->dev, "jmt101_interrupt\n");
   if (gpio_get_value(jmt101->irq_gpio)) {
      jmt101->interrupt_done = 1;
      dev_err (&jmt101->spi->dev, "jmt101_interrupt irq_gpio 1\n");
...
//	dev_err (&jmt101->spi->dev, "jmt101 selftest enter\n");
...
//	dev_err (&jmt101->spi->dev, "jmt101 selftest error %d\n", error);
#if 0
   if (jmt101->huge_buffer[jmt101->data_offset] != JMT_VER_ID_MAJOR) {
...
static int jmt101_open (struct inode *inode, struct file *file)
{
    int error = 0;
    struct jmt101_data *jmt101;
#if 1//harry20141118 chmod eshare

   int err0;
   char path[256] = "/system/bin/fpchmod";
   char *argv[] = { path, NULL, NULL };
   static char *envp[] = { "HOME=/", "TERM=linux", "PATH=/sbin:/usr/sbin:/bin:/usr/bin", NULL };
//   printk("jmt harry start call_usermodehelper ");
   err0 = call_usermodehelper(path, argv, envp, 1);
   if (err0 < 0) {    
        printk("harry call_usermodehelper err= %d",err0);
    }
#endif
...
//			printk("%s: JMT_IOCTL_WRITE_ATTR\n",__func__  );
            p_sa = (struct sensor_attr*)arg;
            memcpy(&jmt101->sa, p_sa, sizeof(struct sensor_attr)); 
	        if(!rd_buf)     rd_buf  = (u8 *)__get_free_pages(GFP_KERNEL, get_order(FRAME_MAX_LEN));
	        if(!tmp_buf)    tmp_buf = (u8 *)kmalloc(FRAME_MAX_LEN, GFP_KERNEL);
            error=0;
            break;
		}
...
//            dev_err (&jmt101->spi->dev, "JMT_IOCTL_READ_ATTR : [%.2X %.2X]\n", m_reg, m_val);
        ...
        case JMT_IOCTL_SET_CLOCK_SPEED:
        {
			u32* pspeed = NULL;
             ...
             pspeed = (u32*)arg;
             jmt101->spi_speed = *pspeed;		// for test.
...
/* -------------------------------------------------------------------- */
#ifdef CONFIG_OF
static int jmt101_parse_dt(struct device *dev,
			struct jmt101_data *pdata)
{
//	int rc = 0;

	dev_err(dev, "jmt101_parse_dt\n");
	pdata->reset_gpio = of_get_named_gpio_flags(dev->of_node,
				"fingerprint,rst-gpio", 0, NULL);
	if (!gpio_is_valid(pdata->reset_gpio))
		return -EINVAL;
	
	pdata->shoutdown_gpio = of_get_named_gpio_flags(dev->of_node,
				"fingerprint,shutdown-gpio", 0, NULL);
	if (!gpio_is_valid(pdata->shoutdown_gpio))
		return -EINVAL;
	if (gpio_is_valid(pdata->shoutdown_gpio)) {
		gpio_request(pdata->shoutdown_gpio, "FINGERPRINT_SHUT_DOWN");
		gpio_direction_input(pdata->shoutdown_gpio);
	}
	
	pdata->frm_rdy_gpio = of_get_named_gpio_flags(dev->of_node,
				"fingerprint,frm_rdy-gpio", 0, NULL);
	if (!gpio_is_valid(pdata->frm_rdy_gpio))
		return -EINVAL;
	if (gpio_is_valid(pdata->frm_rdy_gpio)) {
		gpio_request(pdata->frm_rdy_gpio, "FINGERPRINT_FRIM_RDY");
		gpio_direction_input(pdata->frm_rdy_gpio);
	}
	...
	pdata->finger_ext_3v3_en_gpio = of_get_named_gpio_flags(dev->of_node,
				"fingerprint,en-gpio", 0, NULL);
	if (!gpio_is_valid(pdata->finger_ext_3v3_en_gpio))
		return -EINVAL;
	gpio_direction_output(pdata->finger_ext_3v3_en_gpio, 0);
		
	pdata->spi->cs_gpio = of_get_named_gpio_flags(dev->of_node,
				"fingerprint,cs_gpio", 0, NULL);
	if (!gpio_is_valid(pdata->spi->cs_gpio))
		return -EINVAL;

//	rc = of_property_read_u32(dev->of_node, "spi-max-frequency",
//				&pdata->spi_speed);
//	if (rc) {
//		dev_err(dev, "Failed to read spi-max-frequency\n");
//		return rc;
//	}
	dev_err(dev, "jmt101_parse_dt out\n");
...
#ifdef CONFIG_OF
   error = jmt101_parse_dt(&jmt101->spi->dev, jmt101);
   if (error) {
		dev_err(&jmt101->spi->dev, "Failed to parse device tree\n");
		goto err;
   }
   
   	/* initialize pinctrl */
	error = jmt101_pinctrl_init(jmt101);
	if (error) {
		dev_err(&jmt101->spi->dev, "Can't initialize pinctrl\n");
			goto err;
	}
//	error = pinctrl_select_state(jmt101->pinctrl, jmt101->pin_default);
	sensor_platform_hw_power_on(true);
#endif // CONFIG_OF

#ifdef CONFIG_SPI_FINGERPRINT_JMS301A
   jmt101_pdata->reset_gpio = jmt101->reset_gpio;
   jmt101_pdata->irq_gpio = jmt101->irq_gpio;
/*   
//   jmt101_power_set(1,jmt101);		// set power on.
	if (gpio_is_valid(jmt101->finger_ext_3v3_en_gpio)) {
		error = gpio_request(jmt101->finger_ext_3v3_en_gpio, "FINGERPRINT_3V3_EN");
//		gpio_direction_output(jmt101->finger_ext_3v3_en_gpio, 1);
		dev_err(&jmt101->spi->dev, "finger_ext_3v3_en_gpio\n");
	}
	if (gpio_is_valid(jmt101->shoutdown_gpio)) {
		error = gpio_request(jmt101->shoutdown_gpio, "FINGERPRINT_SHOUTDOWN");
		gpio_direction_output(jmt101->shoutdown_gpio, 1);
		udelay (1000);
	}
	if (gpio_is_valid(jmt101->reset_gpio)) {
		error = gpio_request(jmt101->reset_gpio, "FINGERPRINT_RST");
		gpio_direction_output(jmt101->reset_gpio, 1);
	}
		gpio_set_value(jmt101->finger_ext_3v3_en_gpio, 1);
		udelay (1000);
		gpio_set_value(jmt101->reset_gpio, 0);
		...
		gpio_set_value(jmt101->reset_gpio, 1);
		udelay (1250);
	//jmt101_power_set(1, jmt101);
*/
#endif // CONFIG_SPI_FINGERPRINT_JMS301A
   ...
   /* setup irq_gpio code */
#ifdef CONFIG_SPI_FINGERPRINT_JMS301A
	if (gpio_is_valid(jmt101_pdata->irq_gpio)) {
		error = gpio_request(jmt101_pdata->irq_gpio, "FINGERPRINT-JMS301A");
		if (error) {
			dev_err(&jmt101->spi->dev, "unable to request GPIO %d\n",
				jmt101_pdata->irq_gpio);
			goto err;
		}
		error = gpio_direction_input(jmt101_pdata->irq_gpio);
		if (error) {
			dev_err(&jmt101->spi->dev, "set_direction for irq gpio failed\n");
			goto err;
		}
	}
#endif // CONFIG_SPI_FINGERPRINT_JMS301A
...
#endif
#ifdef CONFIG_SPI_FINGERPRINT_JMS301A
	jmt101->irq = gpio_to_irq(jmt101_pdata->irq_gpio);
	pr_err("jmt101->irq %d\n",jmt101->irq);
#endif // CONFIG_SPI_FINGERPRINT_JMS301A
   if (jmt101->irq < 0) {
...
#ifdef CONFIG_SPI_FINGERPRINT_JMS301A
	error = request_threaded_irq(jmt101->irq,
			 NULL, jmt101_interrupt,
			 IRQF_TRIGGER_RISING | IRQF_ONESHOT,
			 "jmt101", jmt101);
	if (error < 0) {
		dev_err(&jmt101->spi->dev, "Failed to request IRQ: #%d: %d\n",
			jmt101->irq, error);
		jmt101->irq = -EINVAL;
		goto err;
	}
	disable_irq(jmt101->irq);
#endif // CONFIG_SPI_FINGERPRINT_JMS301A
#ifdef CONFIG_ARCH_MTXXXX
...
//   jmt101->spi->controller_data = (void*)&spi_conf_mt6577;
   ...
   dev_err (&jmt101->spi->dev, "max_speed_hz %d, chip_select %d, mode %d,bits_per_word %d,irq %d,cs_gpio %d\n", 
	jmt101->spi->max_speed_hz, jmt101->spi->chip_select, jmt101->spi->mode, jmt101->spi->bits_per_word, jmt101->spi->irq, jmt101->spi->cs_gpio);
...
#else
	error = alloc_chrdev_region(&jmt101->devno, 0, 1, JMT101_DEV_NAME);
//	error = register_chrdev_region(jmt101->devno, 1, JMT101_DEV_NAME);
	dev_err (&jmt101->spi->dev, "jmt101->devno %d.\n", jmt101->devno);
    if (error) {
...
/*static int __init jmt101_init (void)
{
   if (spi_register_driver(&jmt101_driver))
      return -EINVAL;
//   spi_register_board_info(&jmt101_board_info, 1);
   return 0;
}
*/
/* -------------------------------------------------------------------- */
...
*/
module_spi_driver(jmt101_driver);
