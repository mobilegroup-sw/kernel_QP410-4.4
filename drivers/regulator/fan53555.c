...
enum {
	...
	FAN53555_CHIP_ID_04,
	FAN53555_CHIP_ID_05,
	FAN53555_CHIP_ID_08 = 8,		// add lee, 20141229
};
				...
				struct fan53555_platform_data *pdata)
{
	...
	case FAN53555_CHIP_ID_08:		// lee add, 20141229
		di->vsel_min = 600000;
		di->vsel_step = 10000;
		break;
			...
			struct fan53555_device_info *di)
{
	...
	if (of_find_property(node, "fairchild,vsel-old-gpio", NULL)) {
		gpio = of_get_named_gpio_flags(node, "fairchild,vsel-old-gpio", 0,
						&flags);
		...
		ret = devm_gpio_request(di->dev, gpio, "fan53555_old_vsel");
		if (ret) {
			dev_err(di->dev, "Failed to obtain gpio %d ret = %d\n",
				gpio, ret);
			return ret;
		...
		dev_err(di->dev, "Success to set gpio old\n");
	}
