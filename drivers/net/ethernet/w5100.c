...
static irqreturn_t w5100_detect_link(int irq, void *ndev_instance)
{
	...
	if (netif_running(ndev)) {
		if (gpio_get_value(priv->link_gpio) != 1) {
...
static int w5100_mmio_probe(struct platform_device *pdev)
{
	...
	return w5100_probe(&pdev->dev, ops, sizeof(struct w5100_mmio_priv),
			   mac_addr, irq, data ? data->link_gpio : -EINVAL, -EINVAL, -EINVAL);
}
...
int w5100_probe(struct device *dev, const struct w5100_ops *ops,
		...
		int link_gpio, int en_gpio, int level_gpio)
{
...
static int w5100_suspend(struct device *dev)
{
	...
	if (netif_running(ndev)) {
		...
		gpio_set_value(priv->level_gpio, 0);
	}
	return 0;
}
...
static int w5100_resume(struct device *dev)
{
	...
	if (netif_running(ndev)) {
		...
		gpio_set_value(priv->level_gpio, 1);
		w5100_hw_reset(priv);
		w5100_hw_start(priv);
