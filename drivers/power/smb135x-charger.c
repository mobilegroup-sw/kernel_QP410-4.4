...
#define SMB_KMSG_ERR 0
#if SMB_KMSG_ERR
#define SMB_KMSGERR	pr_err
#undef pr_debug
#define pr_debug pr_err
#else
#define SMB_KMSGERR    pr_debug
#endif
...
#define VCHG_OPERATION_BIT		BIT(3)
...
#define TEM_MON_ENABLE_BIT		BIT(6)
...
// LEE ADD FOR TEST.20150327
#define IRQ_1C_REG			0x1C
#define IRQ_G_FAST_CHARGE_CURRENT_MASK		SMB135X_MASK(4, 0)
#define IRQ_49_REG			0x49
...
//	SMB_KMSGERR
	pr_err("%s usb_cur_val %x\n", __func__, usb_cur_val);
	rc = smb135x_masked_write(chip, CMD_INPUT_LIMIT,
							...
							int current_ma)
{
	...
	SMB_KMSGERR("%s, USB current_ma = %d\n", __func__, current_ma);
								...
								int lvl_sel)
{
	...
	SMB_KMSGERR("%s thermal_levels %ud, therm_lvl_sel %ud \n", __func__, chip->thermal_levels, chip->therm_lvl_sel);
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		case POWER_SUPPLY_STATUS_FULL:
			...
			break;
		case POWER_SUPPLY_STATUS_RECHARGING:
			smb135x_masked_write(chip, CMD_CHG_REG, CMD_CHG_EN, CMD_CHG_EN);		// ENABLE CHARGING.
			break;
	...
	// for charging full and also have current from usb to support the device working.
	if ( (prop == POWER_SUPPLY_PROP_STATUS) && (val->intval ==POWER_SUPPLY_STATUS_FULL)) {
//		chip->chg_done_batt_full = false;
		dev_err(chip->dev, "power supply full rc = %d\n", rc);
		rc = smb135x_path_suspend(chip, USB, SOC, false);
		if (rc < 0) {
			dev_err(chip->dev, "Couldn't set usb suspend rc = %d\n",
					rc);
		} else {
			smb135x_masked_write(chip, CMD_CHG_REG, CMD_CHG_EN, 0);
		}	
	}
	return rc;
}
...
/*
bool charger_mode; 
static int __init androidboot_mode_arg(char *options) 
{ 
	if (!strcmp(options, "charger")){ 
		charger_mode = true; 
		pr_err("%s charger mode =%d\n",__func__,charger_mode);	
	}	
	return 0; 
}
early_param("androidboot.mode", androidboot_mode_arg); 
*/
...
/*	rc = smb135x_read(chip, STATUS_5_REG, &reg);
	...
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't read status 42 rc = %d\n", rc);
	}
	
	// 1.poweroff usb charging mode.
	// 2.not battery full statue.
	// 3.usb charging mode(500mA).
	// 4.low charging current.
	// 5.not in otg mode.
	if (charger_mode == true
		&& ( chip->chg_done_batt_full != true)
		&& usb_supply_type == 4
		&& (current_limit == 0 || current_limit == 2 || current_limit == 100)
		&& !(reg & OTG_EN)
		) {
		current_limit = 500;
	}
...
//	SMB_KMSGERR("%s charger_mode = %d, chip->chg_done_batt_full %d\n", __func__, charger_mode, chip->chg_done_batt_full);
//	SMB_KMSGERR("%s current_limit = %d, chip->usb_psy_ma %d\n", __func__, current_limit, chip->usb_psy_ma);
	...
	rc = 0;
	SMB_KMSGERR("%s usb_present = %d, chg_enabled %d\n", __func__, chip->usb_present, chip->chg_enabled);
...
/*	// add for read registers.
	for (status_reg = 0; status_reg < 0x20; status_reg++) {
		rc = smb135x_read(chip, status_reg, &reg);
		dev_err(chip->dev, "read config_registers 0x%x rc = 0x%x\n", status_reg, reg);
	}
	for (status_reg = 0x40; status_reg < 0x57; status_reg++) {
//		if (status_reg == 0x43 ||
//			status_reg == 0x44 || status_reg == 0x45 )
//			continue;
		rc = smb135x_read(chip, status_reg, &reg);
		dev_err(chip->dev, "read status_registers 0x%x rc = 0x%x\n", status_reg, reg);
	}*/
}
...
{
	SMB_KMSGERR("%s chip->version %d\n", __func__, chip->version);
	switch (chip->version) {
	case V_SMB1356:
...
static int recharge_handler(struct smb135x_chg *chip, u8 rt_stat)
{
	...
	if (chip->bms_controlled_charging) {
		smb135x_masked_write(chip, CMD_CHG_REG, CMD_CHG_EN, CMD_CHG_EN);
		rc = smb135x_path_suspend(chip, USB, SOC, false);
		if (rc < 0)
			dev_err(chip->dev, "Couldn't disable usb suspend rc = %d\n",
					rc);
...
//	union power_supply_propval prop = {0,};
	...
	rc = smb135x_masked_write(chip, USBIN_DCIN_CFG_REG, USBIN_BIT5_BIT, USBIN_BIT5_BIT);
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't read status 5 rc = %d\n", rc);
		return rc;
	}
	...
	SMB_KMSGERR("inserted %s, usb psy type = %d stat_5 = 0x%02x\n",
			usb_type_name, usb_supply_type, reg);
	if (chip->usb_psy) {
		if (chip->bms_controlled_charging) {
			/*
		...
		//pr_debug
		SMB_KMSGERR("setting usb psy type = %d\n", usb_supply_type);
//		power_supply_set_supply_type(chip->usb_psy, usb_supply_type);
		power_supply_set_charge_type(chip->usb_psy, usb_supply_type);
		//pr_debug
		SMB_KMSGERR("setting usb psy present = %d\n", chip->usb_present);
		power_supply_set_present(chip->usb_psy, chip->usb_present);
/*		for (status_reg = 0; status_reg < 0x20; status_reg++) {
			rc = smb135x_read(chip, status_reg, &reg);
			dev_err(chip->dev, "read config_registers 0x%x rc = 0x%x\n", status_reg, reg);
...
static int usbin_ov_handler(struct smb135x_chg *chip, u8 rt_stat)
{
	...
	pr_debug("chip->usb_present = %d usb_present = %d\n",
			chip->usb_present, usb_present);
	SMB_KMSGERR("chip->usb_present = %d usb_present = %d\n",
			chip->usb_present, usb_present);
	if (chip->usb_present && !usb_present) {
		/* USB removed */
		chip->usb_present = usb_present;
...
static int smb135x_hw_init(struct smb135x_chg *chip)
{
	...
	// 0x3F = 3000mA(1800mA)
	// 0x3C = 2300mA(1380mA)
	rc = smb135x_masked_write(chip, IRQ_1C_REG, IRQ_G_FAST_CHARGE_CURRENT_MASK, 0x39);
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't set IRQ_1C_REG cmd rc=%d\n", rc);
		return rc;
	}
	// 0x3F = 3000mA
	rc = smb135x_masked_write(chip, IRQ_49_REG, IRQ_G_FAST_CHARGE_CURRENT_MASK, 0x39);
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't set IRQ_49_REG cmd rc=%d\n", rc);
		return rc;
	}
	
	rc = smb135x_masked_write(chip, CFG_1A_REG, TEM_MON_ENABLE_BIT, 0 );
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't read status 1A rc = %d\n", rc);
		return rc;
	}
