...
#define IAVG_SAMPLES			6					// 20150323
#define MIN_SOC_UUC			3

#define QPNP_VM_BMS_DEV_NAME		"qcom,qpnp-vm-bms"

#define QPNP_VM_BMS_DEBUG 0
#if QPNP_VM_BMS_DEBUG
#undef pr_debug
#undef pr_info
...
static int lookup_soc_ocv(struct qpnp_bms_chip *chip, int ocv_uv, int batt_temp)
{
	...
	int iavg_ma_tmp = 0;		// add for test.
	...
	iavg_ma_tmp = calculate_uuc_iavg(chip);		// add for test.
	
	pr_debug("iavg_ma_tmp=%d, current_now %d\n",
			iavg_ma_tmp, chip->current_now);
...
static int report_eoc(struct qpnp_bms_chip *chip)
{
	int rc = -EINVAL;
	union power_supply_propval ret = {0,};
	int status_usb_preset = is_charger_present(chip);
	...
	if (chip->batt_psy) {
		...
		if (rc) {
			...
			if (status_usb_preset == 0)
				return rc; // 在没有连接充电器时，不会上报充电满信息，也就不会点亮绿灯。
...
static void check_recharge_condition(struct qpnp_bms_chip *chip)
{
	...
	/* Report recharge to charger for SOC based resume of charging */	//add 20150724
	if ((status != POWER_SUPPLY_STATUS_CHARGING) && chip->eoc_reported) {
		ret.intval = POWER_SUPPLY_STATUS_RECHARGING;
		rc = chip->batt_psy->set_property(chip->batt_psy,
				POWER_SUPPLY_PROP_STATUS, &ret);
		if (rc < 0) {
			pr_err("Unable to set battery property POWER_SUPPLY_STATUS_RECHARGING rc=%d\n", rc);
		} 
		ret.intval = POWER_SUPPLY_STATUS_CHARGING;
		rc = chip->batt_psy->set_property(chip->batt_psy,
				POWER_SUPPLY_PROP_STATUS, &ret);
		if (rc < 0) {
			pr_err("Unable to set battery property POWER_SUPPLY_STATUS_CHARGING rc=%d\n", rc);
		} else {
...
static void battery_leds_control(struct qpnp_bms_chip *chip)
{
	union power_supply_propval ret = {0,};
//	int status = get_battery_status(chip);
	int status_usb_preset = is_charger_present(chip);

	pr_debug("status_usb_preset %d, chip->battery_full %d\n", status_usb_preset, chip->battery_full);
